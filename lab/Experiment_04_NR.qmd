...
Insert equation [\[eq:v1\]](#eq:v1){reference-type="eqref" reference="eq:v1"} into
...
Insert [\[eq:tf_v2\]](#eq:tf_v2){reference-type="eqref" reference="eq:tf_v2"} into
...
Based on equation [\[eq:v1\]](#eq:v1){reference-type="eqref" reference="eq:v1"}: $$V_1 = -(V_3 + V_4)$$
...
Inserting [\[eq:tf_v3\]](#eq:tf_v3){reference-type="eqref" reference="eq:tf_v3"} and
[\[eq:tf_v4_neg\]](#eq:tf_v4_neg){reference-type="eqref" reference="eq:tf_v4_neg"} into
[\[eq:v1\]](#eq:v1){reference-type="eqref" reference="eq:v1"} results in: 
...
Insert into [\[eq:v1\]](#eq:v1){reference-type="eqref" reference="eq:v1"}: 
...
Based on the correct function for $V_1$ [\[eq:tf_v1_korrekt\]](#eq:tf_v1_korrekt){reference-type="eqref"
...
A comparison of the two transfer functions $\frac{V_2}{V_i}$ (Gleichung
[\[eq:tf_v2\]](#eq:tf_v2){reference-type="eqref" reference="eq:tf_v2"},
[\[eq:V2_aus_V1\]](#eq:V2_aus_V1){reference-type="eqref" reference="eq:V2_aus_V1"})  shows that the error is due to the equation $\frac{V_4}{V_1}$ since the derivation of $\frac{V_1}{V_i}$ remains correct.


**#Conclusion:** Only the use of the negated form of $\frac{V_4}{V_i}$ from equation
[\[eq:tf_v4_neg\]](#eq:tf_v4_neg){reference-type="eqref" reference="eq:tf_v4_neg"} leads to a consistent result for  $\frac{V_1}{V_i}$ according to equation [\[eq:tf_v1_korrekt\]](#eq:tf_v1_korrekt){reference-type="eqref"
reference="eq:tf_v1_korrekt"}. The version from the exercise booklet (equation 
[\[eq:tf_v4_aufg\]](#eq:tf_v4_aufg){reference-type="eqref" 
reference="eq:tf_v4_aufg"}) therefore appears to be incorrect.
